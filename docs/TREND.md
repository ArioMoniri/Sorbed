...
## What it computes (and why these metrics)

Every metric is a real computation over the per-visit analyses, grounded in the
wound-healing literature:

| Metric | Definition | Basis |
|---|---|---|
| **Percent area reduction (PAR)** | `(A₀ − Aₜ) / A₀ × 100` | The single best early predictor of eventual healing. |
...
| **Projected closure** | Linear extrapolation of area → 0 | A first-order estimate; the literature prefers a **log-linear / delayed-exponential** fit (Cukjati 2001) for chronic wounds — a planned refinement. Shown only when the trend is shrinking. |
...
## The honest limitation: no public longitudinal data

We researched this specifically. **There is no verified, public, human, per-patient
*serial* wound-image dataset at scale.** Essentially every public wound dataset —
AZH/FUSeg, the DFU challenges, Medetec, CO2Wounds-V2, the Kaggle sets — is
**cross-sectional** (many wounds, one image each, no visit linkage or dates), even
when the underlying images were collected over years. The only genuinely serial
imaging datasets we could verify are **animal** (murine/porcine), and the human
healing-trajectory studies use **private registry data that is not released**.

The consequence, baked into the design:

- Sorbed ships the **validated deterministic metrics** above. It does **not** ship
  a learned "will-heal" predictor, because there is no public serial data to train
  or validate one honestly. Build your own labeled timeline first (see the roadmap).
- **Calibration and capture consistency across visits is the dominant error
  ...
  is far better but still needs a perpendicular camera and a scale reference at
  *every* visit. Sorbed reports sizes in pixels (not cm²) unless every visit is
  calibrated, and every trend carries a caveat to this effect.

## Roadmap for this feature

- **Now (shipped):** per-visit → timeline model (`WoundTimePoint` → `HealingTrend`),
  PAR, 4-week PAR flag, Gilman rate, PUSH trend, linear projection, the trend
  dashboard, and the `sorbed compare` command.
- **Next:** log-linear / delayed-exponential closure projection with confidence
  bounds; per-etiology thresholds (40% venous / 50% diabetic-foot); DESIGN-R and
  BWAT trend lines; a persisted patient→wound→visit store with EXIF stripping and
  face/tattoo/text redaction for stored timelines.
- **Later:** optional feature-based visit-to-visit registration for overlay/regional
  healing maps (kept approximate and clearly labeled); and — only once real serial
  data exists — an evaluated healing-trajectory model.
