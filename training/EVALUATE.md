...
This directory ships two evaluation surfaces:

- `training/evaluate.py` — the **publication-grade metric harness** (segmentation
  and grading), dependency-light (numpy + scipy only).
- `training/finetune_medsam.py` — fine-tunes SAM/MedSAM for **automatic**
  (prompt-free) wound masks and has a built-in `eval` that puts the fine-tuned
  model head-to-head with the supervised sorbed segmenter.

Everything is patient-aware, reports confidence intervals, and never fabricates a
number. Read `../DISCLAIMER.md` and the honest-scope box in `DATA_README.md` first:
...
- **HD95** (pixels) — 95th-percentile symmetric boundary distance; robust to a few
  ...
  `mm_per_px` (from a fiducial) for millimetres — the harness never guesses scale.
...
correlated — clustering is the honest CI). Optional `prob_<class>` columns for
...
- **Per-stage sensitivity** — recall for each stage, so dangerous under-staging is
  visible per class.
...
- **Calibration** (when probabilities are given) — **ECE**, **Brier**, and a
  **reliability table** (per-bin confidence vs accuracy) emitted as data for
  ...
  skin tone — miscalibration on darker skin is the finding reviewers look for.
...
### What this harness deliberately leaves to you
...
run unattended. This makes them automatic by freezing the heavy image encoder and
...
learn, which fits comfortably on one ~40 GB H200 MIG slice and keeps the export
surface small.
...
the `transformers` SAM/MedSAM path here is the pragmatic, ONNX-friendlier choice.

Offline / air-gapped — pre-download once, then never touch the network:
...
**PROBAST-AI** as described in the methods plan. Always report internal **and**
...
gap are the headline honesty numbers, not the internal best case.
